...
const styles = {
  ...
  exiting: { opacity: 0, transform: getScale(0.8) },
  exited: { opacity: 0, transform: getScale(0.8) },
};

const hiddenStyles = { opacity: 0, transform: getScale(0.8), visibility: 'hidden' };
...
const Grow = React.forwardRef(function Grow(props, ref) {
  ...
  const handleEnter = normalizedTransitionCallback(nodeRef, (node, isAppearing) => {
    ...
    node.style.transition = [
      ...
      theme.transitions.create('transform', {
        duration,
        delay,
        easing: transitionTimingFunction || 'cubic-bezier(0.23, 1, 0.32, 1)',
      }),
    ].join(',');
  ...
  const handleExit = normalizedTransitionCallback(nodeRef, (node) => {
    ...
    node.style.transform = getScale(0.8);
