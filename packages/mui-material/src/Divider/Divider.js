...
const Divider = React.forwardRef(function Divider(inProps, ref) {
  ...
  const resolvedComponent = component || (children || orientation === 'vertical' ? 'div' : 'hr');
  const role = roleProp !== undefined ? roleProp : (resolvedComponent !== 'hr' ? 'separator' : undefined);
  ...
  const dashedSx = dashed ? { borderStyle: 'dashed' } : undefined;
  const mergedSx = dashedSx
    ? Array.isArray(sx) ? [dashedSx, ...sx] : sx ? [dashedSx, sx] : dashedSx
    : sx;
  ...
  return (
    <DividerRoot
      ...
      aria-orientation={
        role === 'separator' && (resolvedComponent !== 'hr' || orientation === 'vertical')
          ? orientation
          : undefined
      }
      sx={mergedSx}
