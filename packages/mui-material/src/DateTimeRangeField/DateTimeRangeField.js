...
const DateTimeRangeFieldRoot = styled('div', {
  name: 'UiDateTimeRangeField',
  slot: 'Root',
})({
  display: 'inline-flex',
  alignItems: 'center',
...
const DateTimeRangeField = React.forwardRef(function DateTimeRangeField(inProps, ref) {
  const props = useDefaultProps({ props: inProps, name: 'UiDateTimeRangeField' });
  const {
    value = [null, null],
    onChange,
  ...
  return (
    <DateTimeRangeFieldRoot ref={ref} sx={{ ...(fullWidth && { width: '100%' }), ...sx }} {...other}>
      <OutlinedInput
        ...
        endAdornment={
          <InputAdornment position="end">
            <IconButton
              ...
              onClick={onIconClick}
              disabled={disabled}
              aria-label="open date time range picker"
            >
              <EventIcon />
            </IconButton>
          </InputAdornment>
