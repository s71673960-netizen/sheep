...
export const style = ({ ownerState, theme }: StyleFunctionProps) => {
  let styles = {
    ...
    ...(ownerState.alignItems && { alignItems: ownerState.alignItems }),
    ...(ownerState.justifyContent && { justifyContent: ownerState.justifyContent }),
    ...handleBreakpoints(
      { theme },
      resolveBreakpointValues({
...
export default function createStack(
  ...
  const Stack = React.forwardRef(function Grid(inProps, ref) {
    ...
    // Merge system props into sx for backward compat with MUI v5/v6 usage
    const systemSx: Record<string, any> = {};
    if (m != null) systemSx.m = m;
    if (mt != null) systemSx.mt = mt;
    if (mr != null) systemSx.mr = mr;
    if (mb != null) systemSx.mb = mb;
    if (ml != null) systemSx.ml = ml;
    if (mx != null) systemSx.mx = mx;
    if (my != null) systemSx.my = my;
    if (p != null) systemSx.p = p;
    if (pt != null) systemSx.pt = pt;
    if (pr != null) systemSx.pr = pr;
    if (pb != null) systemSx.pb = pb;
    if (pl != null) systemSx.pl = pl;
    if (px != null) systemSx.px = px;
    if (py != null) systemSx.py = py;
    if (gap != null) systemSx.gap = gap;
    if (flex != null) systemSx.flex = flex;
    if (flexGrow != null) systemSx.flexGrow = flexGrow;
    if (flexShrink != null) systemSx.flexShrink = flexShrink;
    if (width != null) systemSx.width = width;
    if (height != null) systemSx.height = height;
    if (overflow != null) systemSx.overflow = overflow;

    const hasSysProps = Object.keys(systemSx).length > 0;
    if (hasSysProps) {
      other.sx = other.sx ? [systemSx, ...(Array.isArray(other.sx) ? other.sx : [other.sx])] : systemSx;
    }
